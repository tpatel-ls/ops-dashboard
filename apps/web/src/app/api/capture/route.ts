...
export async function POST(req: Request): Promise<Response> {
  ...
  // This webhook requires an explicit OPS_API_SECRET to be configured.
  // If absent, the endpoint is intentionally disabled to prevent accidental exposure.
  ...
  try {
    ...
    // NOTE: This route intentionally does NOT write to any database.
    // This is a local-first app — there is no server-side DB.
    // Delivering the result to the user's device requires hosting + a real-time sync layer
    // (e.g. Supabase Realtime or a push notification service) that pushes the structured
    // capture to the client, where it can be committed to the local IndexedDB via the
    // normal capture flow.
